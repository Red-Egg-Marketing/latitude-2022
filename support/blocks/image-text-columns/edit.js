...
const EditImageColumns = ( { attributes, setAttributes } ) => {
		const {
			contentAlign, media, title, image, bgColor, bgSlug, color, vidOrImg, videoID, videoURL, withDrop, videothumb, animateScroll
		} = attributes;
		...
		const blockProps = useBlockProps({
			className: 'image-columns' + ' ' + contentAlign + (bgSlug != '' ? ' ' + bgSlug + ' with-bg' : '') + (withDrop == false ? ' no-ds' : ' with-ds') + (animateScroll == true ? ' scroll-activate' : ''),
